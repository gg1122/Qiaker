...
switch($c) {
	case 'index':
		...
		$offset=10;
		$start=($page-1)*$offset;
		$nums=$db->get_one('select count(*) as num from `web_banner`');
		$list=$db->getAll('select * from `web_banner` order by time desc limit '.$start.','.$offset);
		$pagestr=pages_z($nums['num'], $page, $offset);
		include T('admin','banners');
		break;
	case 'add':
		include T('admin','banner_add');	
		break;
	case 'edit':
		$id=intval($_REQUEST['id']);
		$info=$db->get_One('select * from `web_banner` where id='.$id);
		include T('admin','banner_add');
		break;
	case 'del':
		$id=intval($_REQUEST['id']);
		$db->query('delete from `web_banner` where id='.$id);
		showmessage('删除成功','/?m=admin&a=banner');
		break;
	case 'save':
		...
		$infos['time']=time();
		if($id>0){
			$db->update('web_banner',$infos,'id='.$id);
			$msg='更新成功';
		}else{
			$db->insert('web_banner',$infos);
			$msg='发布成功';
		}
		showmessage($msg,'/?m=admin&a=banner');
		break;
}